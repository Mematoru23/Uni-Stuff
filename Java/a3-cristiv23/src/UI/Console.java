...
public class Console {
    ...
    private void addOrder()
    {
        try
        {
            ...
            while (noCakes > 0)
            {
                ...
                cakes.add(cakeService.findById(idCake));
                noCakes--;
            }
    ...
    private void modOrder()
    {
        try
        {
            System.out.println("Which order do you want to modify? Give id: ");
            int id = scanner.nextInt();
            Order order = orderService.findById(id);
            List<Cake> orderCakes = order.getOrderCakes();
            System.out.println("New Order date: ");
            String date = scanner.next();

            orderService.modifyOrder(id, orderCakes, date);
        }
        catch (Exception ex)
        {
            System.out.println(ex.getMessage());
